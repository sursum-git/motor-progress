...
/**
 * ChatDataCollaborator handles all data operations for the Chat component.
 ...
 */
export class ChatDataCollaborator {
    /**
     * Constructs a new ChatDataCollaborator instance.
     * @param chatOptions - Configuration options for the chat widget
     * @param utilsService - Utility service (injected)
     */
    constructor(chatOptions, utilsService = inject(UtilsService)) {
        this.utilsService = utilsService;
        this.options = this.buildOptions(chatOptions);
        this.dataSource = this.createDataSource(chatOptions);
    }
    /**
    ...
    buildOptions(chatOptions) {
        ...
        if (chatOptions.dataSource instanceof kendo.data.DataSource) {
            options = $.extend(true, {}, ChatDataCollaborator.DEFAULT_OPTIONS, chatOptions.dataSource.options);
        }
        else if (Array.isArray(chatOptions.dataSource)) {
            options = $.extend(true, {}, ChatDataCollaborator.DEFAULT_OPTIONS, { data: chatOptions.dataSource });
        }
        else {
            options = $.extend(true, {}, ChatDataCollaborator.DEFAULT_OPTIONS, chatOptions.dataSource);
        }
    ...
    mapFields(fields, chatOptions) {
        for (const key in fields) {
            if (Object.prototype.hasOwnProperty.call(fields, key) && ChatDataCollaborator.FIELD_MAP[key]) {
                const optionKey = ChatDataCollaborator.FIELD_MAP[key];
    ...
    postMessage(message, currentUserId) {
        ...
        const messageData = Object.assign(Object.assign({ id: this.options.autoAssignId ? this.utilsService.guid() : undefined, timestamp: new Date(), files: [] }, messageInput), { authorId: ((_a = messageInput.authorId) === null || _a === void 0 ? void 0 : _a.toString()) || currentUserId });
...
ChatDataCollaborator.DEFAULT_OPTIONS = {
    autoSync: true,
    schema: {
        model: {
            id: "id",
...
ChatDataCollaborator.FIELD_MAP = {
    text: "textField",
