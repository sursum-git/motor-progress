(function () {
  ...
  function onCompanyChanged() {
    ...
    if (companyId) localStorage.setItem(QUERY_COMPANY_KEY, companyId);
    refreshEndpointCombos();
  }
  ...
  function applyEndpoint() {
    const endpoint = (window.SursumContext && typeof SursumContext.getCurrentEnvironment === "function"
      ...
      : null) || endpoints[0];
    if (!endpoint) return;
    if (endpoint.companyId && !localStorage.getItem(QUERY_COMPANY_KEY)) localStorage.setItem(QUERY_COMPANY_KEY, endpoint.companyId);
    const apiBase = window.SursumContext && typeof SursumContext.resolveApiBase === "function"
      ? SursumContext.resolveApiBase(endpoint, selectedCompany())
      : (endpoint.pasoeBaseUrl || endpoint.url || "");
    $("#apiUrl").val(String(apiBase || "").replace(/\/+$/, "") + "/query");
    localStorage.setItem(SELECTED_ENDPOINT_KEY, endpoint.id);
    localStorage.setItem("sursumApiBaseUrl", apiBase || "");
    if (selectedCompany() && selectedCompany().id && !localStorage.getItem(QUERY_COMPANY_KEY)) {
      localStorage.setItem(QUERY_COMPANY_KEY, selectedCompany().id);
    }
  }
  ...
  function loadFromDesigner() {
    ...
    $("#requestPayload").val(formatJson(json));
  ...
  function onFileSelected(event) {
    ...
    reader.onload = function () {
      ...
      $("#requestPayload").val(formatJson(text));
  ...
  function validateRequestPayload() {
    try {
      JSON.parse($("#requestPayload").val());
      $("#parseStatus").text("consulta valida");
      return true;
    } catch (error) {
      $("#parseStatus").text("consulta invalida: " + error.message);
      return false;
    }
  }
  ...
  function runQuery() {
    if (!validateRequestPayload()) {
      setStatus("Corrija a consulta antes de executar.", "error");
      return;
    }
    const payload = $("#requestPayload").val();
    let request = {};
    try { request = JSON.parse(payload); } catch (_) {}
    localStorage.setItem(STORAGE_KEY, payload);
    setStatus("Carregando opcoes de campos e enviando consulta para a API do PASOE...", "");
