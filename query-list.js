(function () {
  ...
  function init() {
    ...
    $("#companyFilter").data("kendoComboBox").value(currentCompanyId() || "TODAS");
  ...
  function loadEndpointConfig() {
    if (window.SursumContext && typeof SursumContext.getConfig === "function") {
      const context = SursumContext.getConfig();
      return { version: 2, companies: Array.isArray(context.companies) ? context.companies : [] };
    }
    let raw = null;
    ...
    return { version: 2, companies: [{ id: "empresa-padrao", name: "Empresa padrao", isDefault: true, endpoints }] };
  }
  ...
  function editQuery(event) {
    ...
    setSelectedCompany(item.companyId || "empresa-padrao");
  ...
  function currentCompanyId() {
    return localStorage.getItem(QUERY_COMPANY_KEY) || localStorage.getItem(SELECTED_COMPANY_KEY) || "TODAS";
  }
