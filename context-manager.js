(function (global) {
  ...
  const WEB_CONTEXT_URL = "context-store.php";
  ...
  function normalizeConfig(raw) {
    ...
    if (raw.version === 4 || Array.isArray(raw.clients)) {
      const normalized = {
        ...
        links: normalizeLinks(Array.isArray(raw.links) ? raw.links : [], Array.isArray(raw.environments) ? raw.environments : []),
  ...
  function normalizePaths(paths) {
    const defaults = DEFAULT_CONFIG.paths || {};
    return {
      metadataRoot: normalizeText(paths.metadataRoot) || defaults.metadataRoot,
      relationsRoot: normalizeText(paths.relationsRoot) || defaults.relationsRoot,
      pasoeRoot: normalizeText(paths.pasoeRoot) || defaults.pasoeRoot,
      databaseCatalog: normalizeText(paths.databaseCatalog) || defaults.databaseCatalog,
      aliasesFile: normalizeText(paths.aliasesFile) || defaults.aliasesFile
    };
  }
  ...
  function normalizeEnvironments(list) {
    ...
    list.forEach((environment, index) => {
      ...
      out.push({
        ...
        servidor: normalizeText(environment.servidor),
        usuario: normalizeText(environment.usuario),
        senha: normalizeText(environment.senha),
        arquivoPf: normalizeText(environment.arquivoPf || environment.arquivo_pf),
        arquivoAlias: normalizeText(environment.arquivoAlias || environment.arquivo_alias)
      });
    });
    return out.length ? out : DEFAULT_CONFIG.environments.slice();
  }

  function normalizeLinks(list, legacyEnvironments) {
    const out = [];
    const used = new Set();
    const add = function (clientId, environmentId) {
      clientId = normalizeText(clientId);
      environmentId = normalizeText(environmentId);
      const key = clientId + "::" + environmentId;
      if (!clientId || !environmentId || used.has(key)) return;
      used.add(key);
      out.push({ id: "link-" + slug(clientId + "-" + environmentId), clientId, environmentId });
    };
    (Array.isArray(list) ? list : []).forEach((link) => add(link.clientId, link.environmentId || link.envId));
    (Array.isArray(legacyEnvironments) ? legacyEnvironments : []).forEach((environment) => add(environment.clientId, environment.id));
    return out;
  }
  ...
  function normalizeCompanies(list) {
    ...
    list.forEach((company, index) => {
      ...
      out.push({
        ...
        code: normalizeText(company.code) || "",
        pathParam: normalizeText(company.pathParam || company.path_param || company.urlCode) || slug(normalizeText(company.name) || id)
      });
    });
  ...
  function ensureRelations(config) {
    ...
    config.links = (config.links || []).filter((link) => clientIds.has(link.clientId) && environmentById[link.environmentId]);
    ...
    const selectedEnvironment = config.environments.find((environment) => environment.id === config.selected.environmentId)
  ...
  function fromLegacyFormat(raw) {
    ...
    const companies = legacyCompanies.map((legacyCompany, index) => {
      ...
      return {
        ...
        code: normalizeText(legacyCompany.code) || "",
        pathParam: normalizeText(legacyCompany.pathParam || legacyCompany.path_param || legacyCompany.urlCode) || slug(legacyCompany.name || `empresa-${index}`)
      };
    });
  ...
  function persistLocal(config) {
    ...
    const company = getSelectedCompany(payload);
    if (environment) localStorage.setItem(LEGACY_BASE_URL_KEY, resolveApiBase(environment, company));
    syncLegacySelection();
    return payload;
  }

  function persist(config) {
    const payload = persistLocal(config);
    saveRemoteContext(payload);
    notifyContextChanged(payload);
  }

  function contextConfigUrl() {
    return WEB_CONTEXT_URL;
  }
  ...
  function isDefaultOnlyConfig(payload) {
    const environments = payload && Array.isArray(payload.environments) ? payload.environments : [];
    const companies = payload && Array.isArray(payload.companies) ? payload.companies : [];
    return environments.length === 1
      && companies.length === 1
      && environments[0].id === "ambiente-local"
      && companies[0].id === "empresa-padrao";
  }

  function saveRemoteContext(payload) {
    const url = state.configUrl || contextConfigUrl();
    if (!url || !global.jQuery) return;
    global.jQuery.ajax({
      url,
      method: "POST",
  ...
  function notifyContextChanged(config) {
    const detail = {
      config,
    ...
    try {
      global.dispatchEvent(new CustomEvent("sursum:context-changed", { detail }));
    } catch (_) {}

    try {
      if (global.parent && global.parent !== global) {
        const targetOrigin = global.location.protocol === "file:" ? "*" : global.location.origin;
        global.parent.postMessage({ type: "sursum:context-changed", detail }, targetOrigin);
      }
    } catch (_) {}
  }
  ...
  function getSelectedEnvironment(config = state.config) {
    return config.environments.find((environment) => environment.id === config.selected.environmentId)
      || config.environments[0];
  }
  ...
  function getEnvironmentsForClient(clientId, config = state.config) {
    const environmentIds = new Set((config.links || []).filter((link) => link.clientId === clientId).map((link) => link.environmentId));
    return config.environments.filter((environment) => environmentIds.has(environment.id));
  }
  ...
  function toLegacyConfig(config) {
    ...
    const companies = config.companies.map((company) => {
      ...
      return {
        ...
        name: company.name,
        code: company.code,
        pathParam: company.pathParam,
        isDefault: selectedCompany && company.id === selectedCompany.id,
        endpoints: [toLegacyEndpoint(environment, company)]
      };
    });
    return { version: 2, companies };
  }

  function toLegacyEndpoint(environment, company) {
    return {
      id: environment.id,
      name: environment.name,
      url: resolveApiBase(environment, company),
      isDefault: true,
      companyId: "legacy"
    };
  }
  ...
  function parseLegacySelection() {
    ...
    const previous = {
      clientId: config.selected.clientId,
      environmentId: config.selected.environmentId,
      companyId: config.selected.companyId
    };
    ...
    if (
      previous.clientId !== config.selected.clientId
      || previous.environmentId !== config.selected.environmentId
      || previous.companyId !== config.selected.companyId
    ) {
      persistLocal(config);
      notifyContextChanged(config);
    }
  }
  ...
  function applyAjaxContext(url, options = {}) {
    ...
    const resolvedBase = resolveApiBase(environment, company);
    const parsed = new URL(url, window.location.href);
    const parsedBase = new URL(resolvedBase.replace(/\/+$/, ""), window.location.href);
  ...
  function setSelection(a, b, c) {
    ...
    if (typeof c !== "undefined") {
      ...
      clientId = normalizeText((company && company.clientId) || config.selected.clientId);
    }

    const selectedClient = config.clients.find((item) => item.id === clientId)
      || config.clients[0];
    const selectedEnvironment = config.environments.find((item) => item.id === environmentId)
      || config.environments[0];
  ...
  function setClientEnvironment(clientId, environmentId) {
    const config = state.config;
    const selectedClient = config.clients.find((item) => item.id === normalizeText(clientId))
      || config.clients[0];
    const selectedEnvironment = config.environments.find((item) => item.id === normalizeText(environmentId))
      || config.environments[0];
  ...
  function setLegacySelection(legacyCompanyId, legacyEndpointId) {
    ...
    const clientId = (company && company.clientId) || config.selected.clientId;
  ...
  function resolveMetadataTargets(aliasName, companyId) {
    ...
    return {
      ...
      companyPathParam: company ? company.pathParam : "",
  ...
  function companyUrlToken(company) {
    return encodeURIComponent(normalizeText(company && (company.pathParam || company.name || company.id)));
  }

  function resolveApiBase(environment, company) {
    const selectedEnvironment = environment || getSelectedEnvironment(state.config);
    const selectedCompany = company || getSelectedCompany(state.config);
    if (!selectedEnvironment) return "";
    const token = companyUrlToken(selectedCompany);
    return sanitizeUrl(String(selectedEnvironment.pasoeBaseUrl || "").replace(/\{empresa\}/gi, token));
  }

  function getCurrentApiBase() {
    parseLegacySelection();
    return resolveApiBase(getSelectedEnvironment(state.config), getSelectedCompany(state.config));
  }
