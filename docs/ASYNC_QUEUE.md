...
A fila async registra o ciclo do job, mas nao substitui uma auditoria geral de execucao/transacao da API.

Melhoria pendente:

- Criar registro unico de execucao/transacao para chamadas `sync`, `async` e workers.
- Registrar contexto operacional, incluindo `requestId`, agente PASOE, PID, client, ambiente, empresa, endpoint, inicio, fim, status e erro.
- Usar esse registro como fonte unica para notificacao de webhook e para endpoint de situacao.
- Persistir tentativas de webhook em tabela propria, com historico de sucesso, falha, retry e erro tecnico.

Detalhamento no roadmap: `docs/ROADMAP_CONSULTAS_SALVAS.md`, secao "Backlog - auditoria de transacoes e notificacao".
...
& 'C:\Progress\OpenEdge\bin\_progres.exe' -b `
  ...
  -p 'sursum-api\workers\RunDynamicQueryClientWorker.p'
```
