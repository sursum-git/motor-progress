...
arquivo request.json
  -> sursum-api/runners/RunDynamicQueryFromJson.p -param request=...;output=...
  -> DynamicQueryRequestSerializer
...
| `sursum-api/runners/RunDynamicQueryFromJson.p` | Runner generico que executa qualquer request JSON recebido por parametro |
...
`boConsDin.p` e `boMetaDados.p` ficam em `sursum-api/sursum/esbo` apenas como referencia.
