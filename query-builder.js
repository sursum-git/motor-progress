(function () {
  ...
  function initWidgets() {
    ...
    $("#externalFilterSource").kendoDropDownList({ dataSource: ["querystring", "header", "body"], value: "querystring" });
    $("#externalFilterOperator").kendoDropDownList({ dataSource: ["=", "<>", ">", ">=", "<", "<=", "between", "begins", "contains"], value: "=" });
    ...
    ["#selectionSource", "#filterSelectionSource", "#externalFilterSelectionSource", "#orderSelectionSource"].forEach((selector) => {
      $(selector).kendoComboBox({
    ...
    $("#pageSize").kendoNumericTextBox({ min: 1, format: "n0", decimals: 0, value: 500 });

    $("#sourceTable").kendoTextBox();
    $("#tableSearchText").kendoTextBox();
    ["#selectField", "#filterField", "#externalFilterField", "#orderField", "#relationLeftField", "#relationRightField"].forEach((selector) => {
    ...
    createGrid("#externalFiltersGrid", state.externalFilters, [
      { field: "name", title: "Parametro" },
      { field: "source", title: "Origem", width: 120 },
      { field: "sourceAlias", title: "Alias", width: 120 },
      { field: "field", title: "Campo" },
      { field: "operator", title: "Op.", width: 80 },
      { field: "required", title: "Obrig.", width: 90 },
      removeColumn("externalFilters")
    ]);
  ...
  function bindEvents() {
    ...
    $("#loadMetadata").on("click", function () {
      loadDatabases(true, function () { loadMetadata(true); });
    });
    ...
    $("#addExternalFilter").on("click", addExternalFilter);
    $("#clearExternalFilters").on("click", function () { state.externalFilters = []; refreshAll(); });
    ...
    $("#generateStepPayload").on("click", buildVisualStepPayload);
    ...
    $("#saveQueryBackend").on("click", saveQueryToBackend);
  ...
  function expandFieldOptions(fields) {
    const expanded = [];
    (fields || []).forEach((field) => {
      const name = String(field.name || field.nome || field.field || "");
      const extent = Number(field.extent || field.extensao || 0);
      if (!name) return;
      if (extent > 0) {
  ...
  function refreshSelectionSourceCombo() {
    ...
    $("#selectionSourceRow, #filterSourceRow, #externalFilterSourceRow, #orderSourceRow").toggle(rows.length > 1);
  ...
  function applyStepThreeSource(source) {
    ...
    $("#externalFilterAlias").val(source.alias || "");
  }

  function selectedAliasFor(selector) {
    const combo = $(selector).data("kendoComboBox");
    const alias = combo ? combo.value() : "";
    const source = (state.sources || []).find((item) => item.alias === alias) || (state.sources || [])[0] || null;
    return source ? (source.alias || source.nome || "") : "";
  }
  ...
  function addExternalFilter() {
    const name = inputValue("#externalFilterName", "").trim();
    const sourceAlias = inputValue("#externalFilterAlias", selectedAliasFor("#externalFilterSelectionSource")).trim();
    const field = comboOrInputValue("#externalFilterField").trim();
    if (!name || !sourceAlias || !field) {
      setStatus("Informe parametro, alias e campo para o filtro externo.", "error");
      return;
    }
    if (!/^[A-Za-z0-9_.-]+$/.test(name)) {
      setStatus("Parametro externo deve usar apenas letras, numeros, _, - ou .", "error");
      return;
    }
    state.externalFilters.push({
      __id: newId(),
      name,
      source: dropdownValue("#externalFilterSource") || "querystring",
      sourceAlias,
      field,
      operator: dropdownValue("#externalFilterOperator") || "=",
      required: $("#externalFilterRequired").is(":checked")
    });
    $("#externalFilterName").val("");
    refreshAll();
  }
  ...
  function saveRelation() {
    ...
    $("#relationPayload").val(JSON.stringify(relation, null, 2));
    ...
    $.ajax({
      ...
      dataType: "json",
      data: JSON.stringify(Object.assign(relationScope(relation.leftTable, relation.leftDatabase || relation.database), {
        source: "manual",
        relations: [relation]
      }))
    }).done(function (response) {
      ...
      const saved = response.data && response.data[0] ? response.data[0] : relation;
      $("#relationPayload").val(JSON.stringify(saved, null, 2));
      setStatus("Relacao salva no SQLite.", "ok");
  ...
  function loadRelation() {
    ...
    $.getJSON(relationStoreUrl(left, relationLeftDatabase()))
      .done(function (response) {
        const relation = findRelationPair(response, left, right, relationLeftDatabase(), relationRightDatabase());
        if (!response.success || !relation) {
  ...
  function addRelationAsJoin() {
    ...
    try {
      const payload = JSON.parse($("#relationPayload").val() || "{}");
      if (payload.leftTable || payload.rightTable) relation = payload;
    } catch (_) {}
  ...
  function fetchSavedRelation(leftSource, rightSource) {
    ...
    $.getJSON(relationStoreUrl(leftSource.nome, leftSource.banco || ""))
      .done(function (response) {
        const relation = findRelationPair(response, leftSource.nome, rightSource.nome, leftSource.banco || "", rightSource.banco || "");
        if (response && response.success && relation) deferred.resolve({ success: true, relation });
  ...
  function relationScope(table, database) {
    ...
    return {
      ...
      database: database || sourceDatabase() || relationLeftDatabase() || "",
      table: table || ""
    };
  }

  function findRelationPair(response, leftTable, rightTable, leftDatabase, rightDatabase) {
    const rows = response && Array.isArray(response.data) ? response.data : [];
    return rows.find((relation) => {
      const sameForward = sameText(relation.leftTable, leftTable)
        && sameText(relation.rightTable, rightTable)
        && sameText(relation.leftDatabase || relation.database, leftDatabase)
        && sameText(relation.rightDatabase || relation.database, rightDatabase);
      const sameReverse = sameText(relation.leftTable, rightTable)
        && sameText(relation.rightTable, leftTable)
        && sameText(relation.leftDatabase || relation.database, rightDatabase)
        && sameText(relation.rightDatabase || relation.database, leftDatabase);
  ...
  function refreshAll() {
    ...
    refreshGrid("#externalFiltersGrid", state.externalFilters);
  ...
  function refreshJson() {
    $("#requestPayload").val(JSON.stringify(buildRequest(), null, 2));
  }
  ...
  function buildRequest() {
    ...
    if (mode === "pipeline") {
      ...
      if (state.externalFilters.length) request.externalFilters = state.externalFilters.map(cleanObject);
      return request;
    }
  ...
  function openResultPage() {
    refreshJson();
    localStorage.setItem(STORAGE_KEY, $("#requestPayload").val());
  ...
  function saveQueryRecord(saveAs) {
    ...
    let request;
    try {
      request = JSON.parse($("#requestPayload").val());
    } catch (error) {
      setStatus("Consulta invalida. Corrija antes de salvar: " + error.message, "error");
      showStep(MAX_STEP);
      return;
    }
    ...
    if (!inputValue("#queryCode", "")) $("#queryCode").val(slugForPath(name));
  ...
  function saveQueryToBackend() {
    refreshJson();
    let request;
    try {
      request = JSON.parse($("#requestPayload").val());
    } catch (error) {
      setStatus("Consulta invalida. Corrija antes de salvar no PASOE: " + error.message, "error");
      showStep(MAX_STEP);
      return;
    }

    const name = inputValue("#queryName", defaultQueryName(request));
    const code = inputValue("#queryCode", slugForPath(name)).trim();
    const status = dropdownValue("#queryStoreStatus") || "draft";
    if (!/^[A-Za-z0-9_.-]+$/.test(code)) {
      setStatus("Codigo da consulta deve usar apenas letras, numeros, _, - ou .", "error");
      return;
    }
    ...
    $.ajax({
      ...
      dataType: "json",
      data: JSON.stringify({ code, status, query: request })
    }).done(function (response) {
      if (!response || !response.success) {
        setStatus("Erro ao salvar no PASOE: " + apiError(response), "error");
        return;
      }
      setStatus("Consulta salva no PASOE como " + code + ".", "ok");
    }).fail(function (xhr) {
      setStatus("Falha HTTP ao salvar no PASOE: " + xhr.status + " " + xhr.statusText, "error");
    });
  }
  ...
  function loadQueryForEdit() {
    ...
    if (id) {
      ...
      if (item.companyId) localStorage.setItem(QUERY_COMPANY_KEY, item.companyId);
      $("#queryName").val(item.name || "");
      $("#queryCode").val(slugForPath(item.name || item.id || "consulta"));
  ...
  function applyRequest(request) {
    ...
    state.externalFilters = withIds(request.externalFilters || []);
    ...
    if (!inputValue("#queryCode", "")) $("#queryCode").val(slugForPath(inputValue("#queryName", defaultQueryName(request))));
