...
function generateCustomers(count) {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    customers.push({
      ...
      ContactEmail: `contact${i}@${account
        .toLowerCase()
        .replace(/\s+/g, "")}.com`,
      ContactPhone: `+1-${Math.floor(Math.random() * 900) + 100}-${
...
const salesPersonBookingsMap = generatedCustomers.reduce((acc, customer) => {
  acc[customer.SalesPerson] =
    (acc[customer.SalesPerson] || 0) + customer.Bookings;
  return acc;
}, {});

// Add SalesPersonBookingsSum to each customer
const customerOrders = generatedCustomers.map((customer) => ({
  ...customer,
  SalesPersonBookingsSum:
    Math.round(salesPersonBookingsMap[customer.SalesPerson] * 100) / 100,
}));
